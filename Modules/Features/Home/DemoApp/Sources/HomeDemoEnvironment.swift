...
@MainActor
final class HomeDemoEnvironment: ObservableObject {
    enum State {
        case loading
        case loaded(useCase: DefaultHomeBusinessUseCase)
    ...
    private func initializeDemoSession() async {
        do {
            ...
            let fetchUseCase = FetchHomeDataUseCaseImpl(
                newsletterRepo: newsletterRepository,
                articleRepo: articleRepository
            )
            let homeUseCase = DefaultHomeBusinessUseCase(fetchUseCase: fetchUseCase)

            state = .loaded(useCase: homeUseCase)
        } catch {
